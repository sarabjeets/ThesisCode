...
public class Prediction {
	...
	public static void main(String[] args)throws IOException {
		...
		//	duration[i] =i+2;
			resask[i] = 20 + (int)(Math.random() * ((250-120) + 1));;// resources userr asks for
...
//		double aveRP = .3;
		...
		Random r = new Random();
		ArrayList<Double> predazam = new ArrayList<Double> ();
		...
		double[] overalluti = new double[10000];
	...
	//	Log.printLine("Starting CloudSimExample1...");
		...
		//	Datacenter datacenter1 = createDatacenter("Datacenter_1");
			...
			userlist.add(user);// add user to user listInteger
			userlist1.add(user1);
	...
	//		cloudletList1 = new ArrayList<Cloudlet>();
		...
		//	user1.submitRequest1(user1.getCloudlet(), false, res, res, (long)(res), numtime, starttime);
			...
			relinp.clear();
			String [] fl = new String[51];
			int[] i2={5,10,20,40,60,100};
			int[] i3={6,11,21,41,61,101};
			String addr = "/home/singh/CsimSj/preDataAvg/r";
			//String m = "y = load('5.txt');m = length(y);x = [1:m];X = [ones(m, 1), x'];theta = pinv(X'*X)*X'*y;";
			//String n =
			
				for(int j=1;j<51;j++)
				{
			      //String file = addr + String.valueOf(j) +"/"+String.valueOf(i2)+".txt";
			         String file = addr + String.valueOf(j) +"/"+"100.txt";
			         fl[j]= file;
			         
				}
			for (int j = 1; j<51; j ++){
				relinp.clear();
			// double[] rp = new double[360];
			     double[] rp = DreadFile(fl[j]);	
		
			  //nested loop           
				 for(int h = 0 ; h < rp.length; h++){
		...
		//	System.out.println(availableNoRelinq + "available ideal res at time" +timestamp[z] );
          ...
          //  System.out.println(availableRelinq + "available relinq res at time" +timestamp[z] );
			...
			double predAzam =  broker.CalNumOfResAzam(user.getuserId(), host.getId(),availableAazam);
			predazam.add(predAzam);
				host.addmtimes(userid);
		...
		//	System.out.println("user" + user1.getuserId() + " asks for " + user.getnumCPU1() + " CPUs and " + user.getnumRAM1() + " GB of RAM and " + user.getnumStorage1() + " GB of storage to host" + host.getId() + "at time" + starttime);
			...
			//	payprice = pledge;
				 ...
				 int id = 0;
				   ...
				   //id = DecideModel(utiliz,rp[z], userid, res,z); 
				   id=1;
			//	   System.out.println("model is" + id);
				...
				double predQi =	broker.CalNumOfRes(user.getuserId(), host.getId(), id,available);
				predqi.add(predQi);
			...
			for (int k = 0; k<predazam.size(); k++){
			     sum1 += predazam.get(k);
			     sum2 += predqi.get(k);
			}
			double avgazam = sum1/50;
			double avgQi = sum2/50;
			double v1 =0;
			double v2 =0;
			double sd1 =0;
			double sd2 =0;
			
			for (int i=0; i < predazam.size(); i++){
            	 v1 += (predazam.get(i)- avgazam)*(predazam.get(i)- avgazam);
            	 v2 +=(predqi.get(i)- avgQi)*(predqi.get(i)- avgQi);
             }
              sd1 = Math.sqrt(v1/predazam.size());
              sd2 = Math.sqrt(v2/predqi.size());
            double  l1 = avgazam - ((1.96* sd1)/Math.sqrt(predazam.size()));
            double  u1 = avgazam  + ((1.96* sd1)/Math.sqrt(predazam.size()));
            
            double  l2 = avgQi - ((1.96* sd2)/Math.sqrt(predazam.size()));
            double  u2 = avgQi + ((1.96* sd2)/Math.sqrt(predazam.size()));
			...
			System.out.println(avgazam + " avg aazam CI " + l1 + u1);
			System.out.println(avgQi + " avg qi CI " + l2 + u2);
		//	payprice = user.getnumCPU() * 0.01 + user.getnumRAM() * 0.01 + (double)(user.getnumStorage()*0.001);
			...
			PrintWriter writerRelinq = new PrintWriter ("C:\\New folder\\Allocated Res Relinq.txt", "UTF-8");
